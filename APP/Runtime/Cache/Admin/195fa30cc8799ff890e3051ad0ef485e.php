...
<tbody>
<?php if(is_array($user)): foreach($user as $key=>$v): ?><tr  class="list-inline">
<td><?php echo ($v["id"]); ?></td>
<td><?php echo ($v["usernamer"]); ?></td>
<td><?php echo (date('y-m-d H:i',$v["logintime"])); ?></td>
<td><?php echo ($v["loginip"]); ?></td>
<td>&nbsp;<?php if($v["lock"]): ?>锁定<?php endif; ?></td>
<td>
<?php if($v["usernamer"] == C("RBAC_SUPERADMIN")): ?>[超级管理员]<?php else: ?>[<?php if(is_array($v["role"])): foreach($v["role"] as $key=>$value): echo ($value["remark"]); endforeach; endif; ?>]<?php endif; ?>
</td>
<td><a>锁定</a></td>
